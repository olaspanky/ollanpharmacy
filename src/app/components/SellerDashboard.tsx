...
// import { Bell, Search, Filter, LogOut, Package, TrendingUp, Users, Clock, Loader2, RefreshCw, X, Grid, Table } from 'lucide-react';
...
//   // Setup SSE connection with proper cleanup and reconnection logic
...
//         // fallback: force refresh to get clean data
//         loadOrders(true);
//         return;
//       }
...
//       }
//     };
//   }, [loadOrders, loadRiders, setupSSE]);
...
//         {/* Cards View */}
//         {viewMode === 'cards' && Object.entries(ordersByDate).map(([date, ordersForDate]) => (
...
export default function SellerDashboard() {
  ...
  // Enhanced seamless update controls
  ...
  const playNotificationSound = () => {
    if (audioRef.current && !silentMode) {
      audioRef.current.currentTime = 0;
      audioRef.current.volume = silentMode ? 0.1 : 0.5; // Quieter in silent mode
      audioRef.current.play().catch(err => {
        console.log('Audio play failed:', err);
      });
    }
  ...
  // Enhanced loadOrders with better caching and less intrusive updates
  const loadOrders = useCallback(async (showRefreshing = false, isBackgroundUpdate = false) => {
    try {
      ...
      // Only show notifications and play sounds if not in background update mode
  ...
  // Enhanced SSE with quieter updates
  const setupSSE = useCallback(() => {
    // Clear any existing connection
    if (sseRef.current) {
      sseRef.current.close();
  ...
  const handleAction = async (
    ...
    try {
      ...
      // Use background update after action to avoid disrupting workflow
